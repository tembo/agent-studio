...
| `API_RESERVED_SUB_AGENT_RUNS` | Slots reserved for sub-agents (default: half of the maximum, so `2` when the maximum is `4`; set `0` if orchestration is unused). |
...
| A run stays queued while other runs are active. | The per-task concurrency cap is full; it starts when a slot opens. Tune `API_MAX_CONCURRENT_RUNS` against task memory. If sub-agents queue behind simultaneous orchestrators, reserve more capacity with `API_RESERVED_SUB_AGENT_RUNS`. |
