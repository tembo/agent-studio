...
export type OutputsSearchParams = Record<
  string,
  string | string[] | undefined
>;

type OutputQueryValues = {
  q: string;
  agent: string;
  orchestrator: string;
  user: string;
...
function paginationHref(
  baseHref: string,
  values: OutputQueryValues,
  cursor: string,
  fixedAgent: boolean,
) {
  const params = new URLSearchParams();
  for (const [key, value] of Object.entries(values)) {
    if (value && !(fixedAgent && key === "agent")) params.set(key, value);
  }
  params.set("cursor", cursor);
  return `${baseHref}?${params.toString()}`;
}

export async function OutputsView({
  workspaceId,
  workspaceSlug,
  baseHref,
  searchParams,
  ...
  const values: OutputQueryValues = {
    q: scalar(searchParams.q).slice(0, 200),
    agent: agentName ?? scalar(searchParams.agent),
    orchestrator: scalar(searchParams.orchestrator),
    user: scalar(searchParams.user),
    from: scalar(searchParams.from),
    to: scalar(searchParams.to),
    delivery: scalar(searchParams.delivery),
  };
  const [page, facets] = await Promise.all([
    listOutputsForWorkspace(workspaceId, {
      ...
      cursor: scalar(searchParams.cursor) || undefined,
    }),
    listOutputFacets(workspaceId),
  ]);
      ...
      <form className="border-border bg-surface flex flex-col gap-3 rounded-xl border p-4">
        <div
          className={cn(
            "grid gap-3",
            agentName
              ? "xl:grid-cols-[minmax(14rem,2fr)_repeat(2,minmax(10rem,1fr))]"
              : "md:grid-cols-[minmax(14rem,2fr)_repeat(3,minmax(10rem,1fr))]",
          )}
        >
          ...
          >
            ...
            {facets.orchestrators.map((orchestrator) => (
              <option key={orchestrator} value={orchestrator}>
                {orchestrator}
              </option>
            ))}
          </FilterSelect>
      ...
      {page.items.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          {page.items.map((output) => (
            <Link
              key={output.runId}
              href={`/${workspaceSlug}/outputs/${output.runId}`}
